...
def ForwardKinemetics( DH_table , JointAngle):
    # 讀取DH參數
    dof = np.size( DH_table , 0 ) 
    theta = np.zeros(dof)
    d = np.zeros(dof)
    a = np.zeros(dof)
    alpha = np.zeros(dof)
    ...
    [EulerAngle[0], EulerAngle[1], EulerAngle[2]] = Rot2RPY.Rot2RPY(Wrist_Center_R)
    Info = [JointPos,JointDir]

    return Info,EulerAngle,Position
